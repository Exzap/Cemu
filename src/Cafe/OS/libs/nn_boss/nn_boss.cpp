...
namespace boss
{
	...
	{
		memset(_thisptr, 0x00, 0x1000);//sizeof(TaskSetting::settings));
	...
	struct Task
	{
		...
		static BossResult Initialize1(Task* _thisptr, const char* taskId, uint32 accountId) // Initialize__Q3_2nn4boss4TaskFPCcUi
		{
		...
		static BossResult Run(Task* _thisptr, bool isForegroundRun)
		{
			...
			serviceCall.WriteParam<uint32be>(_thisptr->accountId);
	...
	struct Storage
	{
		...
		/* +0x0B */ DirectoryName storageName2;
		uint8 ukn13;
		uint8 ukn14;
		uint8 ukn15;
		uint8 ukn16;
		...
		static BossResult Initialize(Storage* _thisptr, const char* storageName, uint32 accountId, StorageKind type)
		{
			...
			cemuLog_logDebug(LogType::Force, "boss::Storage::Initialize({}, 0x{:08x}, {})", storageName, accountId, type);

			_thisptr->storageKind = type;
			_thisptr->titleId.u64 = 0;
			_thisptr->storageName2.name2.assign(storageName);
			_thisptr->accountId = accountId;
			return 0x200080;
		}
		...
		static bool Exist(nn::boss::Storage* _thisptr)
		{
			...
			serviceCall.WriteParam<uint32be>(_thisptr->accountId);
			serviceCall.WriteParam<DirectoryName>(_thisptr->storageName2);
		...
		static nnResult GetDataList(nn::boss::Storage* _thisptr, DataName* dataList, sint32 maxEntries, uint32be* outputEntryCount, uint32 startIndex) // GetDataList__Q3_2nn4boss7StorageCFPQ3_2nn4boss8DataNameUiPUiT2
		{
			...
			bool uknByte = serviceCall.ReadResponse<uint8be>() != 0;
			cemu_assert_debug(uknByte); // probably indicates if all entries fit into the output buffer?
			if (outputEntryCount)
	...
	static_assert(offsetof(Storage, storageName2) == 0x0B);
	...
	struct NsData
	{
		...
		static BossResult Initialize(NsData* _this, nn::boss::Storage* storage, const char* dataName)
		{
			...
			_this->storage.storageName2 = storage->storageName2;
			...
			cemuLog_logDebug(LogType::Force, "initialize: {}", _this->name.c_str());
			return 0x200080;
		}
		...
		static std::string _GetPath(NsData* nsData)
		{
			...
			path /= nsData->storage.storageName2.name2.c_str();
		...
		static BossResult DeleteRealFile(NsData* _thisptr)
		{
			...
			serviceCall.WriteParam<uint32be>(_thisptr->storage.accountId);
			serviceCall.WriteParam<DirectoryName>(_thisptr->storage.storageName2);
