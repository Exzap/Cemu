...
namespace GX2
{
	...
	void GX2Command_StartNewCommandBuffer(uint32 numU32s)
	{
		...
		while (true)
		{
			// try to grab buffer data from either of two places:
			...
			uint32 readIndex = GX2Command_GetPoolGPUReadIndex();
			...
			// but this has false negatives due to race conditions because the last submission timestamp is updated independently of the read index
