...
// todo - Use NAPI instead of using CURL directly?
...
namespace iosu::boss
{
	...
	public:
		struct BossStorageFadEntry
		{
			...
			uint32 memo_2C; // 0x2C
			uint32 ukn30; // 0x30
			uint32be timestampRelated; // 0x34 - guessed. 0x30 seems to be part of the timestamp? At least 0x30 and 0x34 are usually set together
			// flags:
			// 0x80000000		ReadFlag
		...
		nnResult AddEntry(CafeString<32> fileName, uint32 fileId)
		{
			...
			for (sint32 i=0; i<FAD_ENTRY_MAX_COUNT; i++)
			{
				if (m_fadData.entries[i].fileId == 0)
				{
					...
					m_fadData.entries[i].timestampRelated = 0;
					m_fadData.entries[i].ukn30 = 0;
	...
	public:
		...
		bool Exists() const
		{
			...
			if (m_nsDataPath.empty())
				return false;
			return fs::exists(m_nsDataPath, ec);
		}
		...
		nnResult TaskDoRequest(CURL* curl)
		{
			...
			if (m_taskSettings.taskType == TaskType::NbdlTaskSetting)
			{
				...
				for (const auto& file : queuedFiles)
					cemuLog_log(LogType::Force, "\t- {} ({}KB)", file.fileName, (file.size+1023)/1024);
				for (const auto& file : queuedFiles)
				{
	...
	private:
			...
			for (pugi::xml_node sheet = doc.child("TaskSheet"); sheet; sheet = sheet.next_sibling("TaskSheet"))
			{
				...
				for (pugi::xml_node file = files.child("File"); file; file = file.next_sibling("File"))
				{
					...
					if (strcmp(type.child_value(), "AppData") == 0)
						...
						fileType = 0; // unsure what the default is? It may also not be recognized as a valid file
		...
		void DownloadNbdlFile(const NbdlQueuedFile& nbdlFile)
		{
			...
			std::vector<uint8> receivedData = request.getReceivedData(); // todo - clear internal buffer here?
			...
			AES128CTR_transform(receivedData.data() + offsetof(BossNbdlHeader, encryptedHeader), sizeof(BossNbdlHeader::encryptedHeader), bossAesKey, aesNonce);
			AES128CTR_transform(receivedData.data() + sizeof(BossNbdlHeader), receivedData.size() - sizeof(BossNbdlHeader), bossAesKey, aesNonce);
			// write to filesystem
			uint64 titleId = CafeSystem::GetForegroundTitleId(); // todo - use titleId from task settings?
			fs::path path = ActiveSettings::GetMlcPath("usr/boss/{:08x}/{:08x}/user/common/data/{}", (uint32)(titleId >> 32), (uint32)(titleId & 0xFFFFFFFF), m_taskId.id.c_str());
			if (!fs::exists(path))
				fs::create_directories(path);
			path.append(fmt::format("{:08x}", nbdlFile.dataId));
			...
			{
				cemuLog_log(LogType::Force, "Failed to create SpotPass nbdl file: {}", _pathToUtf8(path));
				return;
			}
			// update or create FAD database
	...
	public:
		...
		void RegisterTask(const TaskSettingCore& taskSetting)
		{
			...
			{
				__debugbreak(); // task already registered. What error to return?
				return;
			}
	...
	public:
		...
		// helper function to get persistentId from account slot
		std::optional<uint32> BossGetPersistentId(uint32 accountSlot)
		{
			if (accountSlot == 0)
				return iosuAct_getAccountIdOfCurrentAccount();
			uint32 persistentId;
		...
		{
			std::optional<uint32> persistentId = BossGetPersistentId(serviceCall.ReadParameter<uint32be>());
			if (!persistentId)
		...
		nnResult HandleTaskStopScheduling(IPCServiceCall& serviceCall)
		{
			...
			if (!persistentId)
				return RESULT_C0203880;
			TaskId taskId = serviceCall.ReadParameter<TaskId>();
			cemuLog_logDebug(LogType::Force, "IOSU-Boss: Unsupported StopScheduling called for task {}", taskId.id.c_str());
			return RESULT_SUCCESS;
		}
		...
		nnResult HandleStorageGetDataList(IPCServiceCall& serviceCall)
		{
			...
			serviceCall.WriteResponse<uint8be>(1); // extra byte to indicate failure or success? This may indicate if the list did fit or not. But if we dont return 1 then GetDataList returns an error
			return RESULT_SUCCESS;
		}
